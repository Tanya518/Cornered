...
while playing:
    ...
    for event in pygame.event.get():
        ...
        if event.type == CREATE_SNOW_CIRCLE:
            snow_circles.append(create_snow_hexagon())
        if event.type == CREATE_GREEN_CIRCLE:
            green_circles.append(create_green_hexagon())
    ...
    for enemy in enemies:
        ...
        if player_rect.colliderect(enemy[1]) and not enemy_collision_occurred:
            # Mix sarcastic and curse phrases for variety
    ...
    for diamond in diamonds:
        ...
        if player_rect.colliderect(diamond[1]) and not diamond_collision_occurred:
            # Diamonds deserve extra sarcasm
            current_phrase = random.choice(SARCASTIC_PHRASES)
            phrase_timer = 120
    ...
    for snow_circle in snow_circles:
        snow_circle[3] += 0.25  # Швидша вібрація
        vibration_amplitude = 3.5  # Різкіша вібрація
    ...
    for green_circle in green_circles:
        green_circle[3] += 0.25  # Швидша вібрація
        vibration_amplitude = 3.5  # Різкіша вібрація
        ...
        if player_rect.colliderect(green_circle[1]):
            current_phrase = random.choice(HAPPY_PHRASES)
            phrase_timer = 120
            score += 3  # Зелений шестикутник дає 3 бали
            is_snowing = False
            wind_force = pygame.math.Vector2(0, 0)
            snowflakes.clear()
...
restart_text = FONT.render('Press R to Restart or Q to Quit', True, COLOR_GREEN)
